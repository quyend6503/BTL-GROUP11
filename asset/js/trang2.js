...
// RÀNG BUỘC HỌ TÊN
    ...
    // THÔNG BÁO HOÀN TẤT
    const bookingHTML = `
  <p><strong>Họ tên</strong><span>${name}</span></p>
  ...
  <p>Giờ <span>${time}</span></p>
  <p>Bàn <span>${table}</span></p>
`;

showBookingPopup(bookingHTML);
}
...
function showBookingPopup(html) {
    document.getElementById("bookingContent").innerHTML = html;
